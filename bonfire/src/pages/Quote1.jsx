...
const RepairForm = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await fetch('http://192.168.1.9:1337/api/quotes', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ "data": { "Name": formData.get('name'), "Phone": formData.get('phone'), "Email": formData.get('email'), "DeviceType": formData.get('device-type'), "DeviceName": formData.get('device-name') } }), });
